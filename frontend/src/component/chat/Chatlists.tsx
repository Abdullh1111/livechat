...
const Chatlists = () => {
  ...
  return (
    <div
      className={`bg-[#FFF] max-h-[90vh] md:flex flex-col w-full 2xl:w-[20%] md:w-[40%] overflow-auto rounded-xl p-5 ${
        chatWith.name ? "hidden" : ""
      }`}
    >
      ...
      {isLoading ? (
        ...
        data?.data
          ?.filter((data: TUser) => data?.email !== user?.email)
          .map((data: TUser, index: number) => (
